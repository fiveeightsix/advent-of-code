...
impl Image {
    ...
    pub fn expand_column(&mut self, x: usize, factor: usize) {
        let new_columns = factor - 1;
        
        self.width += new_columns;
            
        for galaxy in self.galaxies.iter_mut() {
            if galaxy.x > x {
                galaxy.x += new_columns;
            }
        }
    }

    pub fn expand_row(&mut self, y: usize, factor: usize) {
        let new_rows = factor - 1;
    ...
    pub fn expand(&mut self, factor: usize) {
        ...
        while x < self.width {
            if self.is_empty_column(x) {
                self.expand_column(x, factor);
                x += factor; // skip over expanded column
            } else {
                x += 1;
            }
        }
        ...
        while y < self.height {
            if self.is_empty_row(y) {
                self.expand_row(y, factor);
                y += factor; // skip over expanded row
            } else {
                y += 1;
            }
        }
...
";
        ...
        assert_eq!(1030, image.get_total_distance());
    }
...
";
        ...
        assert_eq!(8410, image.get_total_distance());
    }

    #[test]
    fn image_expand_column_sample() {
        let expected_image = Image::from_input(r".#..#.
");
        let mut image = Image::from_input(r".#.#.
");

        image.expand_column(2, 2);
